...
<template>
    <main>
      ...
      <table>
        ...
        <tr><td>Vraag:</td><td><textarea v-model="form.question" rows="4" cols="80"/></td></tr>
        <tr><td>A</td><td><textarea v-model="form.answerA" rows="2" cols="80"/></td></tr>
        <tr><td>B</td><td><textarea v-model="form.answerB" rows="2" cols="80"/></td></tr>
        <tr><td>C</td><td><textarea v-model="form.answerC" rows="2" cols="80"/></td></tr>
        <tr><td>D</td><td><textarea v-model="form.answerD" rows="2" cols="80"/></td></tr>
        ...
        <tr><td>Brevet 2</td><td>
            <select v-model="form.allowB2">
                <option value="true">Ja</option>
                <option value="false">Nee</option>
            </select>
        </td></tr>
        <tr><td>Brevet 3</td><td>
            <select v-model="form.allowB3">
                <option value="true">Ja</option>
                <option value="false">Nee</option>
            </select>
        </td></tr>
        <tr><td>Negeren</td><td>
            <select v-model="form.ignore">
                <option value="true">Ja</option>
                <option value="false">Nee</option>
            </select>
        </td></tr>
        <tr><td>Bespreken</td><td>
            <select v-model="form.discuss">
                <option value="true">Ja</option>
                <option value="false">Nee</option>
            </select>
        </td></tr>
        <tr><td>Opmerkingen</td><td><textarea v-model="form.remarks" rows="4" cols="80"/></td></tr>
        <tr><td>Examengroep</td><td><input v-model="form.examGroup"/></td></tr>
        <tr><td>Afbeelding</td><td>
            <select v-model="form.pictureId">
  ...
  export default {
      ...
      mounted() {
        ...
        if (this.questionId) {
            axios.get('/api/questions/' + this.questionId).then((response) => { this.form = response.data });
        }
    }
  }
  </script>